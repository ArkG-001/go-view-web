<template>
  <n-modal class="go-chart-data-monaco-editor" :value="showModal" :mask-closable="false" :closeOnEsc="false">
        ...
        <n-space justify="space-between">
          <div>
            ...
            <n-data-table
              ref="dataTableRef"
              max-height="420px"
              style="margin-top: 8px; height: 420px"
              :row-key="(row:any) => row.k"
              virtual-scroll
...
const saveFilter = () => {
  ...
  if (!checkedRowKeys.value.length) {
    window['$message'].error('请选择点位')
    return
  }
  // targetData.value.filter = filter.value
  console.log(checkedRowKeys.value[0], 'checkedRowKeys')
  targetData.value.filter = `return data.data.data.workData.find(item => item.k === '${checkedRowKeys.value[0]}').v`
  closeFilter()
}
